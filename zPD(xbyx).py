...
def current_time(timezone):
    ...
    return dt, time, day
def extract_hs(name,current_symbols,current_prices,current_ma_prices,dt_list):
    with open('zworkspace/results/'+name+'.txt', 'r') as f:
        bbox = f.read().strip()
...
def combine(lst):
    ...
    for item in lst:
        output += item
    return output
def check_trend(symbol,x,prices,dt_list,points_x,points_y,usx,usy,detection_ln,trend_ln):
    #return True
    ...
    for symb in current_symbols:
        ...
        if name == "":
            ...
            name += ","+symb
    file_name = "zworkspace/save/"+name+".jpg"
    txt_name = "zworkspace/results/"+name+".txt"
    result_file_name = "zworkspace/results/"+name+".jpg"
    ...
    for tsymbol in current_symbols:
        name += tsymbol+","
    name = name[:-1]
    x_dict,y_dict,class_dict = extract_hs(name,current_symbols,current_prices,current_ma_prices,dt_list)
    os.remove(file_name)
    #os.remove(txt_name)
    ...
    for change in changes:
        if abs(change) > 1*height_trend:
            print(change,height_trend)
            new_changes.append(change)

    #print(new_changes)
    #print(changes)
    print(height_trend - combine(new_changes))
    vol_adjusted_height_flag = True#height_trend - combine(new_changes) > 1.5*(head - min(t1,t2))
    print(vol_adjusted_height_flag)
...
parser.add_argument('--weights', nargs='+', type=str, default='PD_weights_new/weights4_1_2.pt', help='model.pt path(s)')
...
with torch.no_grad():
    ...
    if input("clear files") == "True":
        ...
        clear_jpg_files("zworkspace/save","txt")
    ...
    while True:
        ...
        if (day != "Sunday" or day != "Saturday") and '09:30:00' < now < '16:00:00' or True:
            ...
            og_watchlist = read_json_file('MS yolo/zworkspace/watchlist.txt',watchlist)
            ...
            if watchlist != {}:
                ...
                for symbol in symbols:
                    ...
                    #print(dt_list)
                    if valid_flag and str(start) in dt_list and len(lst) < 180:
                        ...
                        if iteration == dimension**2:
                            ...
                            if True:
                                ...
                                for key in current_keys:
                                    ...
                                    check_trend(symbol_name,x_trend_lst,y_trend_lst,dt_list,x,y,unshortened_x,unshortened_y,max_detection_len,max_trend_len)
                                    #---------------
                                    """
                                            ...
                                            for repeat_num in range(repeats):
                                                ...
                                                if determine_valid(x,[left_shoulder,t1,head,t2,right_shoulder],end_x,end):
                                                    if len(after_y_list) > 0:
                                                        ...
                                                        after_bool = True
                                                    after_bool = True#REMOVE
                                                    print(after_y_list)
                                                    print(end < min(left_shoulder,right_shoulder), after_bool)
                                                    if (end < min(t1,t2)) and after_bool:
                                                        ...
                                                        for i in range(len(y_trend_lst)):
                                                            x_trend_lst.append(i)
                                                        if check_trend(symbol_name,x_trend_lst,y_trend_lst,dt_list,x,y,unshortened_x,unshortened_y,max_detection_len,max_trend_len):
                                                            print("developed")
                ...
                for tmp in new_false_list:
                    source_path = "zworkspace/current/"+tmp+".jpg"
                    destination_path = "zworkspace/false/"+tmp+".jpg"
                    if not(os.path.exists(destination_path)):
                        shutil.copyfile(source_path, destination_path)
                    if tmp in watchlist.keys():
                        del watchlist[tmp]
                current_watchlist = read_json_file('MS yolo/zworkspace/watchlist.txt',watchlist)
                temp_remove = []
            ...
            jpg_files = glob.glob(os.path.join("zworkspace/results", '*.jpg'))
            txt_files = glob.glob(os.path.join("zworkspace/results", '*.txt'))
            ...
            if flag:
                ...
                if time.time()-start_time < 5:
                    time.sleep(int(5-(time.time()-start_time)))
            else:
                time.sleep(20)
